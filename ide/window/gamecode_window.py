...
def create_center_region(root, application_context):
    ...
    development_view = PanedWindow(center, bg='white', orient=HORIZONTAL, sashwidth=8, showhandle=True, height=600)
    project_hierarchy = ProjectHierarchyFrame(development_view, application_context)
    code_editor_portion = Frame(development_view, bg='green')
    code_editor = CodeEditorFrame(code_editor_portion)
    development_view.add(project_hierarchy)
    development_view.add(code_editor_portion)
